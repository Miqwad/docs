...
| **SpringMockK** (`com.ninja-squad:springmockk`) | **5.0.1** | ✅ **confirmed on Boot 4.1 / JDK 25** at scaffold | No known high/critical (OSV, 2026-06-30) | github.com/Ninja-Squad/springmockk | `@MockkBean`/`@SpykBean` for Spring tests. |
| **Testcontainers** (`org.testcontainers:*`) | **2.0.5** (own BOM) | ✅ **adopted at scaffold** — not on the Boot 4.1 BOM, so import `testcontainers-bom:2.0.5` | No known high/critical (OSV, 2026-06-30) | github.com/testcontainers/testcontainers-java | **2.0 renamed the modules** to `testcontainers-postgresql` / `testcontainers-junit-jupiter`, and `PostgreSQLContainer` moved to `org.testcontainers.postgresql` and is **no longer generic**. |
...
| **Detekt** | **`dev.detekt` 2.0.0-alpha.3** | ✅ **adopted at scaffold** — runs on JDK 25; stable 1.23.x can't even start on a JDK-25 daemon. Plugin id moved to **`dev.detekt`** (the old `io.gitlab.arturbosch.detekt` id ships only 1.x) | No known high/critical (OSV, 2026-06-30) | detekt.dev; github.com/detekt/detekt | Lets the **whole build run on JDK 25** (no JDK-21 daemon). It's an alpha — pin a stable 2.0 at GA. |
| **ktlint** | **via `dev.detekt:detekt-rules-ktlint-wrapper`** | ✅ | No known high/critical (OSV, 2026-06-30) | detekt.dev; github.com/pinterest/ktlint | In detekt 2.0 the wrapper artifact is `detekt-rules-ktlint-wrapper` and the config ruleset key is **`ktlint`** (renamed from `formatting`). |
...
## Verified building the walking skeleton (2026-06-30) — F-5 done

The BOM was exercised end-to-end building the [backend skeleton](https://github.com/Miqwad/backend) (CI-green on JDK 25). What bit, recorded so it doesn't bite again:

- **The whole build runs on JDK 25.** Adopting detekt **`dev.detekt` 2.0** removed the JDK-21 Gradle daemon that detekt 1.23.x would otherwise force (1.23.x can't run on a JDK-25 daemon).
- **Boot 4 modularization moved packages** (a starter no longer drags everything in): `HibernatePropertiesCustomizer` → `org.springframework.boot.hibernate.autoconfigure`; `@AutoConfigureMockMvc` → `org.springframework.boot.webmvc.test.autoconfigure`, and it needs a test-scoped **`spring-boot-starter-webmvc-test`** (no longer in `spring-boot-starter-test`).
- **JobRunr** auto-config is split — excluding it in tests means excluding **both** `JobRunrAutoConfiguration` and `JobRunrSqlStorageAutoConfiguration`.
- **Dependency management** is Gradle-native BOM import (`platform(SpringBootPlugin.BOM_COORDINATES)`), not the `io.spring.dependency-management` plugin.
- **ZATCA SDK** signs on JDK 25 (F-6 ✅) but its fat jar bundles Spring → load it isolated (F-7; see [integrations.md](../architecture/integrations.md)).
- Confirmed working on Boot 4.1 / JDK 25 as used: **SpringMockK 5.0.1**, **Resilience4j `-spring-boot4` 2.4.0**, **JobRunr 8.7.0**, **Testcontainers 2.0.5**.
