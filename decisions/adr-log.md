...
**Consequences.** One component library across surfaces; ~2 effective frontend builders. The React-vs-Next choice is isolated to the web portals — it does not affect the customer app, the design tokens, or the API — and should be resolved with a short spike before frontend scaffolding (STATUS O-1).
