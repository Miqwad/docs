...
> Companion docs: [project-plan.md](project-plan.md) (sprints, milestones, risks), [roles.md](roles.md). Owners: **SB** senior backend · **MB** mid backend · **SF** senior frontend · **UX** UI/UX design-engineer · **MBA**.
...
| **B3 Package-driven onboarding** *(MB/SF)* — admin assigns package; dealer UI gated | Management-only dealer has no marketplace nav; server blocks it too |
...
## EPIC C — Dealership onboarding & fleet · *S1, MB + SF/UX*
...
## EPIC D — Availability & marketplace search · *S2, SB + MB + SF/UX*
...
| **D3 Marketplace search + listing + quote** *(MB/SF)* — PostGIS radius/nearest, filters, itemized quote | p95 ≤ 800ms on the test fleet; only `marketplace_listing`-entitled dealers appear |
| **D4 Customer app shell + search/listing UI** *(SF/UX)* | RN app boots with RTL + gated nav; search returns live inventory |

## EPIC E — Migration / import · *S2 fleet/customers/bookings, S5 history — MB + SF/UX + MBA*

| Story | Acceptance criteria |
|---|---|
| **E1 Import module + staging + validation/quarantine** *(MB)* | bad rows quarantined, batch not failed; idempotent re-commit |
| **E2 Templates (vehicle/customer/booking) + import wizard UI** *(SF/UX)* | dealer bulk-loads fleet; counts verified |
...
## EPIC F — Booking loop & channels · *S3, SB + MB + SF/UX* → 🔵 M2
...
## EPIC G — Handover/return, ZATCA, deposits · *S4, SB + MB + SF/UX*
...
| **H2 Telemetry ingest + partitions (pg_partman via JobRunr) + live map + history** *(SB/MB/SF)* | live map renders from Redis; dedup of out-of-order pings; partition maintenance job runs |
| **H3 Wasl adapter** *(SB)* | telemetry feeds Wasl in the provider-agnostic shape (Togglz-gated) |

## EPIC I — Finance, settlement, delivery, one-way · *S5, SB + MB + SF/UX*
...
| **I2 Delivery & collection scheduling** *(MB/SF)* | schedule with fee + status flow |
| **I3 One-way rentals** *(MB/SF)* | pickup ≠ return branch supported end-to-end |
| **I4 Finance/settlement UI + plan/billing screens** *(SF/UX)* | — |

## EPIC J — Breadth: notifications, reviews, reports, Saher, wizard · *S6, all*

| Story | Acceptance criteria |
|---|---|
| **J1 Notifications (FCM/SMS/email) + comms log** *(SB/MB/SF)* | lifecycle events notify; every send persisted |
| **J2 Ratings & reviews** *(MB/SF)* | post-completion review affects listing ranking |
| **J3 Dealer reports + PDF/Excel (read replica)** *(MB/SF/UX)* | reports query the replica, never primary |
| **J4 Saher traffic-violation passthrough** *(MB/SF)* | fine in rental window attributed to renter on record |
| **J5 Branch permissions + consolidated reporting** *(MB/SF)*; **J6 Self-service connection wizard** *(SB/SF)* — credentials to vault, Togglz-gate; **J7 Admin KPIs** *(MB/SF)* | — |
...
Deep-design docs reviewed; schema migrates on Testcontainers; CI green on the skeleton; entitlement gating proven on a sample module; design system covers core components; **gov sandbox credentials applied** (Moyasar + Tajeer expected by S1–S3); **packaging matrix signed off**; GCP/CNTXT procurement underway (O8). Only then does the W1 clock start.
